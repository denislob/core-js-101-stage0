...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    const elem = str[i];
    if (str.indexOf(elem) === i && str.lastIndexOf(elem) === i) {
      return elem;
    }
  }
  return null;
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const outcome = `${openBracket}${Math.min(a, b)}, ${Math.max(a, b)}${closedBracket}`;
  return outcome;
}
...
function isCreditCardNumber(ccn) {
  const nums = Array.from(String(ccn), Number).reverse();
  const sum = nums.reduce((accum, num, index) => {
    let temporary = num;
    if (index % 2 === 1) {
      temporary = num * 2;
      if (temporary > 9) temporary -= 9;
    }
    return accum + temporary;
  }, 0);
  return sum % 10 === 0;
}
...
function getCommonDirectoryPath(pathes) {
  let outcome = '';
  const arrs = pathes.map((elem) => elem.split('/'));
  const len = Math.min(...pathes.map((elem) => elem.length));
  for (let i = 0; i < len; i += 1) {
    const temporary = arrs[0][i];
    if (arrs.every((elem) => elem[i] === temporary)) {
      outcome += `${temporary}/`;
    } else {
      break;
    }
...
function getMatrixProduct(m1, m2) {
  const outcome = Array.from({ length: m1.length }, () => []);
